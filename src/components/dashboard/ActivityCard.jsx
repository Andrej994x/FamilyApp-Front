    ...
    <div className="bg-white rounded-lg p-4 lg:p-6 border border-gray-300">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <button className="w-6 h-6 lg:w-8 lg:h-8 rounded-full border border-gray-300 flex items-center justify-center hover:bg-gray-50 cursor-pointer">
      ...
      {activity.hasEvent && (
        <div className={`${activity.eventColor} rounded-lg p-4 lg:p-5 flex items-start space-x-3 lg:space-x-4 mt-3`}>
          <div className="text-center min-w-[40px] lg:min-w-[50px]">
            <div className={`text-xs ${activity.eventTextColor} opacity-75 font-medium`}>{activity.eventMonth}</div>
            <div className={`text-xl lg:text-2xl font-bold ${activity.eventTextColor}`}>{activity.eventDate}</div>
          </div>
          <div className="flex-1">
            <h3 className={`font-medium ${activity.eventTextColor} mb-1 text-sm lg:text-base`}>{activity.eventType}</h3>
            <p className={`text-xs lg:text-sm ${activity.eventTextColor} opacity-75 mb-2`}>{activity.eventDetails}</p>
        ...
        <div className="flex items-start">
          ...
          <div className="flex-1">
            <textarea
              ...
              className="w-full bg-gray-50 rounded-lg px-4 py-2 focus:outline-none border border-gray-200 text-gray-500 text-sm resize-none"
